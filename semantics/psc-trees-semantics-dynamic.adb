...
package body PSC.Trees.Semantics.Dynamic is
   ...
   Max_Code_Length : Code_Length_Type := 5000;

   --  Maximum PSVM instructions per block within operation
   Max_Block_Length : Code_Length_Type := 2500;
   pragma Warnings (On);
   ...
   begin
      ...
      then
         ...
         --  Mentioning a var means the entry value
   ...
   begin
      ...
      case T.Operator is
         when Stmt_Ops =>
            --  Operands not evaluated in parallel unless Parallel_Stmt_Op
            Eval_In_Parallel := (T.Operator = Parallel_Stmt_Op);
      ...
      if Eval_In_Parallel then
         ...
         begin
            ...
            if T.Operator = Parallel_Stmt_Op then
               ...
               --  Parallel_Stmt_Op always requires a master
   ...
   begin
      ...
      if All_Elems_Are_Refs (T.Annotations) then
   ...
   begin  --  Emit
      ...
      case Instr.Op is
         when Start_Parallel_Op | Start_Parallel_Call_Op
            ...
            --  NOTE: Store_Operation_Desc_Op doesn't really need one
            --        currently, but may in the future.
   ...
   begin  --  Finish_Cur_Inst_Param_Info
      ...
      --  Finish cur-inst param info for type descriptors that are
      --  waiting to evaluate nested constants, etc.
   ...
   --  after following Originating_Operation and Equiv_To links
      ...
      Routine_To_Use : Interpreter.Routine_RW_Ptr := Op_To_Use.Routine;
   begin
   ...
   begin
            ...
            begin
               if Formal_Sem.all in Param_Semantic_Info then
                  ...
                  begin
                     ...
                     --  We use negative addresses for type descriptors
                     Type_Desc.Parameters (I).Data.Value :=
                       -(Word_Type
                          (Actual_Type_Desc.Index) * Chunk_Divisor) - 1;
                  end;
               end if;
            end;
         end loop;
      ...
      --  See whether there are any nested constants
      Type_Desc.Num_Nested_Consts :=
        Natural (Num_Elements (Assoc_Module.Nested_Constants));

      if Obj_Type.All_Parameters_Known
        and then
          (Type_Desc.Num_Nested_Consts > 0
   ...
   begin
      ...
      --  Re-initialized Num_Nested_Consts in case module wasn't quite
      --  finished having code generated when type desc was first created.
      Type_Desc.Num_Nested_Consts :=
        Natural (Num_Elements (Assoc_Module.Nested_Constants));

      if Type_Desc.Num_Nested_Consts > 0 then
         --  Fill in info on nested constants
         Type_Desc.Nested_Consts :=
           new Const_Info_Array (1 .. Type_Desc.Num_Nested_Consts);

         if Debug_Code_Gen then
            Put_Line (" Building info on nested constants:");
         end if;

         for I in 1 .. Type_Desc.Num_Nested_Consts loop
            declare
               Const_Sem : constant Sem_Ptr :=
                 Nth_Element
                    (Assoc_Module.Nested_Constants, Sem_Info_Index (I));
            begin
               if Const_Sem.all in Sym_Reference_Info'Class then
                  ...
                  begin
                     ...
                     --  Copy info
                     Type_Desc.Nested_Consts (I).Data :=
                        Nth_Type_Area_Element
                          (From_Type_Desc,
                           From_Locator.Offset);
                     ...
                     --       for the constant
                     Type_Desc.Nested_Consts (I).Name :=
                        Strings.String_Lookup
                  ...
                  --  Evaluate it now.
                  declare
                     Nested_Const : constant Object_Sem_Ptr :=
                       Object_Sem_Ptr (Const_Sem);
                     Initial_Value : constant Optional_Tree :=
                       Obj_Decl.Tree (Tree_Ptr_Of
                         (Nested_Const.Definition).all).Obj_Value;
                  ...
                  begin
                     ...
                     Type_Desc.Nested_Consts (I).Name :=
                        Strings.String_Lookup
                          (Strings.To_String (Type_Desc.Name) & "::" &
                            Sym_Name (Nested_Const.Associated_Symbol));

                     Evaluate_Tree
                       (Resolved_Tree (Resolved_Initial_Value),
                        Decl_Region =>
                          Nested_Const.Associated_Symbol.Enclosing_Region,
                        Enclosing_Type => Type_Desc,
                        Dest_Name => Strings.Index
                          (Type_Desc.Nested_Consts (I).Name),
                        Value => Type_Desc.Nested_Consts (I).Data.Value,
                        Addr => Type_Desc.Nested_Consts (I).Data.Addr,
                        Type_Desc =>
                          Type_Desc.Nested_Consts (I).Data.Type_Desc);
      ...
      end if;  --  Has at least one nested constant
                           ...
                           then
                              ...
                              begin
                                 ...
                                 if Nearest_Enc_Module /= null then
                                    ...
                                    Add_Element
                                      (Nearest_Enc_Module.Nested_Constants,
                                       Sem_Ptr (Obj_Ref),
   ...
   is
      ...
      --  Store_Type_Related_Const_Op into a temp, and return
      --  locator for temp.
      use Interpreter;
      ...
      when Type_Area | Enclosing_Type_Areas =>
         ...
         then
            ...
            begin
               ...
               then
                  --  We have a constant from a named type which is not
                  --  an enclosing type, so we need
                  --  to create a temp copy of the constant,
                  --  and use Store_Type_Related_Const_Op.
                  declare
                     Temp_Const : constant Offset_Within_Area :=
                       Current_Visitor.Target_Local_Offset;
                     Temp_Const_VM_Info : constant VM_Obj_Id_Type :=
                       Assign_VM_Obj_Id (Current_Visitor.all,
                         Needs_Var => True);
                  begin
                     if Debug_Code_Gen then
                        Put_Line ("Creating temp for " &
                          ...
                          " at (Local_Area," &
                          Offset_Within_Area'Image (Temp_Const) & ')');
                     end if;
                     ...
                     Emit
                       (Current_Visitor.all,
                        (Declare_Obj_Op,
                         Source_Pos => Src_Pos,
                         Destination =>
                           (Local_Area, Temp_Const, Temp_Const_VM_Info),
                         Dest_Name => Strings.Null_U_String_Index,
                         Is_By_Ref => False,
                         Is_Var => False,
                     ...
                     Emit
                       (Current_Visitor.all,
                        (Store_Type_Related_Const_Op,
                         Source_Pos => Src_Pos,
                         Destination => (Local_Area, Temp_Const,
                                         Temp_Const_VM_Info),
                         Dest_Name => Strings.Null_U_String_Index,
                         Type_Info => Run_Time_Type_Info
                           (Obj_Ref.Resolved_Type,
                         ...
                         Source_Type_Info => Run_Time_Type_Info
                           (Prefix_Type,
                            ...
                            For_Val_Param_Or_Const =>
                              ...
                              or else
                                Obj_Location.Offset in
                                   Type_Nested_Const_Offsets)));

                     --  Return the locator for the temp
                     return (Local_Area, Temp_Const, Temp_Const_VM_Info);
                  end;
               end if;
            end;
         end if;
   ...
   begin  --  Emit_Nested_Block_Start
      ...
      case Instr_Opcode is
         when Start_Parallel_Op =>
            if Debug_Code_Gen then
               Put_Line
                 (" Initiate parallel block, Master_Is_Started = " &
   ...
   procedure Emit_Nested_Block
     (Orig_Visitor : in out Code_Gen_Visitor;
      Comp_Sem : Computation_Sem_Ptr;
      Block_Region : Region_Ptr;
      Statement_Tree : Optional_Tree) is
      --  Emit invocation of nested block (used for operands of "||").
   ...
   begin
      ...
      if Case_Alt_Key_Tree in Param_Decl.Tree then
         declare
            ...
            Choice_Param_Type : constant Type_Sem_Ptr :=
         ...
         begin
            ...
            Choice_Param_Sem.Info.Obj_Location :=
              (Local_Area, Choice_Param_Ref_Offset, Choice_Param_VM_Info);
            ...
            if Choice_Param_Type.Is_Polymorphic then
               ...
               pragma Assert (not Choice_Param_Sem.Is_By_Ref_Choice_Param);
               null;
            else
     ...
     (Loop_Visitor : in out Code_Gen_Visitor;
      ...
      begin
         if Iter_Sem.Enclosing_For_Loop.Enclosing_Container_Agg_Sem /= null
           and then Iter_Sem.Iterator_Direction = Interpreter.Unordered_Dir
         then
            declare
      ...
      then
         ...
         declare
            ...
            Call_Tree : Invocation.Tree renames Invocation.Tree
              (Tree_Ptr_Of (Call_Sem.Equiv_Invocation).all);
         ...
         begin
            ...
            if Visitor.Current_Code = null then
               ...
               if New_Obj.Kind = Object_Sym_Kind then
                  --  This must be a global constant.
                  --  Assign it a location in the type descriptor
                  declare
                     Constant_Index : Natural;
                  begin
                     pragma Assert (not Static.Sym_Is_Variable (New_Obj));

                     if Enc_Module = null
                       or else Enc_Module.Cur_Inst_Sem.All_Parameters_Known
                       or else Static.Is_Compile_Time_Known
                     ...
                     then
                        ...
                        --  Add to list of nested constants of enclosing module
                        Add_Element
                          (Enc_Module.Nested_Constants,
                           Sem_Ptr (Obj_Sem),
                           Sem_Info_Index (Constant_Index));
   ...
   begin
      if Visitor.Current_Code = null
        ...
        or else All_Elems_Are_Refs (T.Annotations)
      then
         --  Nothing to do
         return;
      ...
      case T.Kind is
         ...
         when Return_Stmt =>
            ...
            if Not_Null (T.Values) then
               ...
               --  Allocate space for finalizable temps, if any.
               Allocate_Finalizable_Temps
                 (Visitor, Num_Finalizable_Temps (T.Values));
               ...
               begin
                  ...
                  if Output_Is_By_Ref then
                     ...
                     Check_Is_Part_Of_Ref
                       (T.Values, Enclosing_Operation.Nested_Region);
                     ...
                     --  Evaluate address
                     Emit_Code_For_Resolved_Tree (T.Values, Visitor);
                     ...
                     Emit
                       (Visitor,
                        (Store_Address_Op,
                         ...
                         Source => Visitor.Lvalue_Location,
                         Might_Be_Null =>
                           Resolved_Type (T.Values).Value_Is_Optional));
                     ...
                     Emit
                       (Visitor,
                        (Copy_Word_Op,
                         ...
                         Dest_Name => Strings.Index
                           (Output_Sem.Associated_Symbol.Str),
                         Source => (Local_Area, Result_Offset, Temp_VM_Info),
                         Might_Be_Null => True));
                  ...
                  --  Remember high-water and cut back target_local_offset
