...
package body PSC.Univ_Strings is
   ...
   type Indicator_Type is mod 2**Word_Type'Size;
   subtype Univ_String_Index is Indicator_Type;
   ...
   Indicator_Multiplier    : constant Indicator_Type := 2**(24 + 32);
      --  Amount to multiply negated indicator to produce Univ_String value

   Null_Value_Indicator    : constant Indicator_Type := 2;
      --  NOTE: This needs to be consistent with Large_Null_Chunk
   pragma Assert (-Null_Value_Indicator * Indicator_Multiplier =
      Indicator_Type (To_Unsigned_Word (Interpreter.Large_Null_Chunk) *
        Interpreter.Chunk_Divisor));
   ...
   Chars_Per_Word : constant := Interpreter.Word_Type'Size / Character'Size;
         ...
         Underlying_Array_Type_Desc :=
           Type_Descriptor_Ops.Get_Type_Desc_By_Name
             (Strings.String_Lookup
               (Languages.Optional_Univ_Integer_Basic_Array_Type_Name));
      end if;
   ...
   --  Return Indicator "buried" in Univ_String
   begin
      return 2**8 - S2I (Str) / Indicator_Multiplier;
               ...
               when Array_8 =>
                  ...
                  begin
                     for Word_Index in 1 .. Arr_Len loop
                        ...
                        for I in 0 ..
                          Natural'Min (Chars_Per_Word - 1,
                                       Str_Len - Index)
                        loop
                           declare
                              Next_Elem : constant Word_Type :=
                                Next_Word mod 2 ** Character'Size;
                           begin
                              Result (Index + I) :=
                                Character'Val (Next_Elem);
                              Next_Word :=
                                 (Next_Word - Next_Elem) / 2 ** Character'Size;
                           end;
                        end loop;
                        Index := Index + Chars_Per_Word;
      ...
      if Len <= Max_Short_String then
         ...
         declare
            Word_Len : constant Natural :=   --  TBD: Handle wide characters
              (Len + Chars_Per_Word - 1) / Chars_Per_Word;
         ...
         begin
            ...
            for I in 1 .. Word_Len loop
               ...
               for J in reverse 0 ..
                 Natural'Min (Chars_Per_Word - 1, Str'Last - Index)
               loop
                  if Next_Word >=
                    2 ** (Word_Type'Size - Character'Size - 1)
                  then
                     --  Will overflow; make it negative
                     Next_Word := Next_Word -
                       2 ** (Word_Type'Size - Character'Size);
                     pragma Assert (Next_Word < 0);
                  end if;

                  Next_Word := Next_Word * (2 ** Character'Size) +
   ...
   begin
      ...
      if Str < 0 and then Str mod 2 = 1 then
         ...
         begin
            ...
            when Array_8 =>
               declare
                  Limit : constant Natural := Arr_Len * Chars_Per_Word;
                        ...
                        for New_Index in New_String'First .. End_Index loop
                           declare
                              ...
                              Word_Index : constant Positive := 1 +
                                (Index_In_Result - 1) / Chars_Per_Word;
                              Char_Offset_In_Word : constant Natural :=
                                (Index_In_Result - 1) mod Chars_Per_Word;
                              Shift_Amount : constant Natural :=
                                Char_Offset_In_Word * Character'Size;
                           ...
                           begin
                              ...
                              if Char_Offset_In_Word = Chars_Per_Word - 1
                                and then Chr >= 2 ** (Character'Size - 1)
                              then
                                 --  Shift will overflow; negate
                                 Shifted_Chr := (Chr - 2 ** Character'Size) *
                                   2 ** Shift_Amount;
                              else
                  ...
                  if Space < Added_Len then
                     ...
                     declare
                        ...
                        Addon_Arr_Len : constant Natural := Natural'Max
                          (Arr_Len,
                           (Added_Len - Space + Chars_Per_Word - 1) /
                             Chars_Per_Word);
   ...
   begin
      ...
      Assign_Concat_String (Str_Ptr, To_String (New_String), Server_Index);
               ...
               when Array_8 =>
                  declare
                     Word_Index : constant Positive :=
                       (Index - 1) / Chars_Per_Word + 1;
                     Within_Word_Offset : constant Natural :=
                       (Index - 1) mod Chars_Per_Word;
                     Divisor : constant Word_Type :=
                       2 ** (Character'Size * Within_Word_Offset);
                  ...
                  begin
                     if Desired_Word < 0 then
                        ...
                        return (Desired_Word - Desired_Word mod Divisor) /
                          Divisor mod 2 ** Character'Size;
                     else
                        return Desired_Word / Divisor mod 2 ** Character'Size;
                     end if;
                  end;
               when Array_16 =>
                  declare
