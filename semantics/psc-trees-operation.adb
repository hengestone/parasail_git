...
package body PSC.Trees.Operation is
   ...
   begin
      ...
      case T.Operation_Kind is
         ...
         when Op_Operation =>
            Put (On, "op ");
         when Proc_Operation | Proc_Type_Specifier =>
            Put (On, "proc ");
         when Func_Operation | Func_Type_Specifier =>
            Put (On, "func ");
      end case;

      Display_Subtree (T.Name, On);

      if Length (T.Operation_Inputs) > 0
      ...
      if not Use_Short_Form and then Not_Null (T.Preconditions) then
         if Length (T.Operation_Inputs) > 1 or else Indent > 0 then
            New_Line (On, Indent => Indent + 2);
         else
            Put (On, ' ');
         end if;
      ...
      if Length (T.Operation_Outputs) > 0 then
      ...
      if not Use_Short_Form and then Not_Null (T.Postconditions) then
         if Length (T.Operation_Inputs) > 1
   ...
   begin
      if T_Source_Pos = Null_Source_Position then
         ...
         if T_Source_Pos = Null_Source_Position
           and then Lists.Length (T.Operation_Inputs) > 0
         then
            T_Source_Pos :=
