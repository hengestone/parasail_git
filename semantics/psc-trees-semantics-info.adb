...
package body PSC.Trees.Semantics.Info is
   ...
   begin
      ...
      then
         ...
         return False;
      elsif Base_Type1.Associated_Module /=
            Base_Type2.Associated_Module
      then
         --  Not same module
         --  (that's weird since these tables are module-specific!)
         if Debug_Second_Pass then
            Put_Line
              (" Module mismatch in ""Same_Actual_Parameters"":" &
               Sym_Name (Base_Type1.Associated_Module.Associated_Symbol) &
               " /= " &
               Sym_Name (Base_Type2.Associated_Module.Associated_Symbol));
         end if;
         return False;
...
begin
   ...
   Func_Type_Module.Num_Module_Parameters := 0;
   Func_Type_Module.Num_Constructors := 0;
   Func_Type_Module.Num_Visible_Components := 0;
   Func_Type_Module.Is_Interface := True;

   Func_Type_Module.Associated_Symbol := new Symbols.Symbol'
     (Kind => Symbols.Module_Sym_Kind,
      ...
      Nested_Region => null,
      Sem_Info => Root_Sem_Ptr (Func_Type_Module),
   ...
   Func_Type_Module.Definition := Module.Make
     (Name => Identifier.Make (Func_Type_Module.Associated_Symbol.Str),
   ...
   Func_Type_Module.Associated_Symbol.Definition :=
     Func_Type_Module.Definition;
