...
set valid_flags = "o:O:cm:gikb:lvdhtxan:"
...
while ("$1" != "--")
   switch ($1)
      ...
      case "-n":
         ...
         switch ($inlining)
           case "0":
             set psl_flags = ($psl_flags "--inline=0")
             breaksw
           case "1":
             set psl_flags = ($psl_flags "--inline=14")
             breaksw
           case "2":
             set psl_flags = ($psl_flags "--inline=20")
             breaksw
           default:
      ...
      case "-a":
         ...
         set psl_flags = ($psl_flags '"'"--checks"'"')
         breaksw
      case "-l":
...
set psl_flags = ($psl_flags "$verbose_flg")
...
if ($bootstrap == 4) then
   ...
   set to_be_linked = ($parascope_srcs $parascope_main $files)
   set to_be_compiled = ($parascope_main $files)
   set make_target = "compiled_main_with_interp"
