...
if "$to_be_comp" != "" then
   ...
   echo ../build/bin/parasail_main $flags ../lib/aaa/aaa*.ps? ../lib/reflection.ps? \
      ../lib/llvm_printer.ps? ../lib/compiler.ps? $srcs -command Compile $to_be_comp
   ../build/bin/parasail_main $flags ../lib/aaa/aaa*.ps? ../lib/reflection.ps? \
      ../lib/llvm_printer.ps? ../lib/compiler.ps? $srcs -command Compile $to_be_comp
endif

set objs = ""
foreach i (../lib/aaa/aaa*.ps?.o $*)
   if "$i:r.o" == "$i" then
      #  already compiled
      if "$i" != "../lib/aaa/aaa*.ps?.o" then
         if "$i:h" == "../lib/aaa" then
            set objs = ($objs lib/$i)
         else
