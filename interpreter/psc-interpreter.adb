...
package body PSC.Interpreter is
   ...
   package Thread_Server_Info is
      ...
      type Master_Extra_Rec is record
         ...
         Owned_By_Server : Thread_Server_Index'Base := 0;
      ...
      Max_Dynamically_Allocated_Thread_Servers : Natural := 4;
   ...
   type Reclamation_Info_Record (Modulus_Minus_One : Hash_Value) is record
   ...
   package PSVM_Thread_Support is
      ...
      function Prepare_To_Exit
        (Thread_Master    : Word_Ptr;
         ...
         Exiting_Tcb      : Word_Ptr) return Boolean;
   ...
   package Debug is
      ...
      procedure Dump_Type_Element_Data
        (Data   : Element_Info;
         Indent : String := "");
      --  Dump the data associated with a nested constant or parameter
   ...
   begin
      if Doing_Run_Time_Checks then
         ...
         begin
            if Is_Null_Value (Val, Get_Type_Desc (Context, Type_Info)) then
   ...
   is
      ...
      Op_Desc : constant Object_Virtual_Address :=
         Create_Large_Obj (Op_Desc_Type_Desc, Local_Stg_Rgn (Context),
            Context.Server_Index);
   ...
   --  Initialize Global_Data_Stg_Rgn if not yet done
   begin
            ...
            if Old_Type_Desc.Type_Kind = Basic_Array_Kind then
               ...
               declare
                  Arr_Comp_Type : constant Type_Descriptor_Ptr :=
                    Old_Type_Desc.Parameters (1).Data.Type_Desc;
               begin
      ...
      if Source_Type_Desc.Is_Polymorphic then
         ...
         declare
            Poly_Type : constant Type_Descriptor_Ptr :=
                          To_Type_Desc
                            (Large_Obj_Type_Info (Source_Obj_To_Use));
   ...
   is
      ...
      Poly_Type  : constant Type_Descriptor_Ptr :=
                     To_Type_Desc (Large_Obj_Type_Info (Poly_Obj));
      pragma Assert (Poly_Type.Is_Polymorphic);
      ...
      if Enclosing_Lock /= 0 then
         ...
         declare
            ...
            pragma Assert (not Is_Small
              (To_Type_Desc (Large_Obj_Type_Info
                (Large_Obj)).Components (1).Type_Desc));
         begin
   ...
   protected body Thread_Manager is
      ...
      entry Prepare_To_Exit
        (Thread_Master    : Word_Ptr;
         ...
         Holding_Lock_Obj : Lock_Obj_Index;
         Exiting_Tcb      : Word_Ptr;
         Succeeded        : out Boolean) when True
      --  Prepare to exit given master.
      --  Set Succeeded to indicate whether the prepare-to-exit
      --  succeeded.
      is
   ...
   package body Type_Descriptor_Ops is
      ...
      begin
         ...
         case Item_Offset is
            ...
            when Type_Nested_Const_Offsets'Range =>
               --  Nested constant
               Item_Index :=
                 Natural (Item_Offset - Type_Nested_Const_Offsets'First);
               if not Type_Desc_To_Use.Is_Finished
                 and then Type_Desc_To_Use.Nested_Consts (Item_Index).Data.Addr
                             = Null_Virtual_Address
               then
                  --  Make sure type descriptor is fully initialized
                  Finish_Type_Descriptor (Type_Desc_To_Use);
               ...
               return Type_Desc_To_Use.Nested_Consts (Item_Index).Data;
      ...
      begin
         ...
         case Item_Offset is
            ...
            when Type_Nested_Const_Offsets'Range =>
               ...
               Item_Index :=
                 Natural (Item_Offset - Type_Nested_Const_Offsets'First);
               return Type_Desc_To_Use.Nested_Consts (Item_Index).Data.Value;
      ...
      begin
         ...
         case Item_Offset is
            ...
            when Type_Nested_Const_Offsets'Range =>
               ...
               return Type_Desc_To_Use.Nested_Consts (Item_Index).Data.Addr;
      ...
      begin  --  Reconstruct_Type_Descriptors
         ...
         --  the arrays of Params, Actuals_Of_Formals, and Nested_Consts
         for I in 1 .. Num_Entries loop
            if Type_Desc_Is_Incomplete (I) then
               if Debug_Type_Descs then
   ...
   package body Debug is
      ...
      begin
         ...
         if Type_Desc_To_Use = null then
            ...
            Type_Desc_To_Use := To_Type_Desc (Large_Obj_Type_Info (Value));
         ...
         if Is_Large_Null (Value) then
            ...
            begin
               ...
               if Type_Desc_To_Use.Type_Kind = Basic_Array_Kind then
                  --  dump "Basic_Array" components
                  declare
                     Comp_Type : constant Type_Descriptor_Ptr :=
                       Type_Desc_To_Use.Parameters (1).Data.Type_Desc;
                     Len : constant Word_Type :=
                       Content_Of_Virtual_Address
      ...
      begin
         ...
         if Type_Desc.Has_Op_Map then
            ...
            if Type_Desc.Nested_Consts /= null then
               Put_Line (" Nested constants:");

               for I in Type_Desc.Nested_Consts'Range loop
                  Put_Line
                    ("  const " &
                     Strings.To_String (Type_Desc.Nested_Consts (I).Name) &
                     ":");
                  Dump_Type_Element_Data
   ...
   is
      ...
      Element_Type : constant Type_Descriptor_Ptr :=
                       Array_Type_Desc.Parameters (1).Data.Type_Desc;
   begin
      --  Fill in header
      Set_Large_Obj_Header
        (New_Obj,
      ...
      if Dest_Type_Desc.Is_Polymorphic then
         ...
         begin
            ...
            then
               ...
               --  Create a "null" obj
               Store_Word (Dest_Addr, 0, Null_Virtual_Address);
               if Debug_Calls then
...
--       end if;
      ...
      loop
         ...
         begin
            ...
            when Store_Type_Related_Const_Op =>
               --  Store the value of a nested constant or some other
   ...
   --        We also require special large values to be odd.
      subtype Special_Chunk_Value_Range is Unsigned_Word_Type
        range (Large_Null_Base_Value - 1) * 4 .. Large_Null_Base_Value - 1;
   begin
   ...
   begin
      ...
      if U_Type_Desc.Type_Kind = Basic_Array_Kind then
         ...
         declare
            Arr_Comp_Type : constant Type_Descriptor_Ptr :=
              U_Type_Desc.Parameters (1).Data.Type_Desc;
   ...
   begin
      ...
      if U_Type_Desc.Type_Kind = Basic_Array_Kind then
         ...
         if U_Type_Desc.Is_Polymorphic then
            ...
            U_Type_Desc := Get_Large_Obj_Type_Descriptor (Object);
   ...
   begin  --  Wait_For_Open_Master
      ...
      Wait_For_Threads
        (Master_Address,
         ...
         Holding_Lock_Obj,
         New_Local_Area,
         Tcb_Waiting => Context.Control_Area);
