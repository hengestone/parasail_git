...
-- Compound delimiters --
%token L_BRACE -- "{"    -- left brace 
%token R_BRACE -- "}"    -- right brace 
...
interface_declaration_with_term : 
   ...
   OUTDENT_opt_NEWLINE R_BRACE_NEWLINE {
      declare
 ...
 | opt_interface_qualifier CLASS_kw INTERFACE_kw module_defining_name 
     formals_and_implemented_interfaces
     L_BRACE_INDENT
      interface_element_list
      opt_annotation_opt_term
   ...
   OUTDENT_opt_NEWLINE R_BRACE_NEWLINE {
      declare
	Elem_List : Lists.List := $7.List;
      begin
	if $1.Is_Private and then $5.Has_Module_Formals then
	    yyerror("Private interface may not add module parameters");
	end if;
	if not Lists.Is_Empty($8.List) then
	    -- Include the opt_annotation
	    Lists.Append(Elem_List, Annotation.Make(Annotations => $8.List));
	end if;
	$$ := (One_Tree, Tree => PSC.Trees.Module.Make(
	  ...
	  Is_Concurrent => $1.Is_Concurrent,
	  Is_Limited => False,
	  Has_Formals => $5.Has_Module_Formals,
	  Module_Formals => $5.Module_Formals,
	  Extends_Interface => $5.Extends,
	  Implements_Interfaces => $5.Implements,
	  Class_Locals => Lists.Empty_List,
	  Module_Exports => Elem_List,
	  Module_New_Exports => $9.List,
	  Module_Implements => $10.List));
      end;
   }
 | opt_interface_qualifier INTERFACE_kw module_defining_name 
     formals_and_implemented_interfaces
     L_BRACE_NEWLINE R_BRACE_NEWLINE {
	if $1.Is_Private and then $4.Has_Module_Formals then
	    yyerror("Private interface may not add module parameters");
	end if;
	$$ := (One_Tree, Tree => PSC.Trees.Module.Make(
...
value_formal : value_formal_without_mode { $$ := $1; }
  | value_formal_mode value_formal_without_mode {
        $$ := $2;
        declare
            First_Item : Param_Decl.Tree renames Param_Decl.Tree
              (Tree_Ptr_Of (Lists.Nth_Element($2.List, 1)).all);
        begin
            First_Item.Kind := $1.Param_Kind;
        end;
    }
  ;

value_formal_without_mode :
    type_specifier id 
...
polymorphic_type_name : simple_name '+' {
...
qualified_name : 
  ...
  | qualified_name '.' id_or_string_literal {
...
annotated_type_specifier : 
    non_func_annotated_type_specifier { $$ := $1; }
  | func_type_specifier {
        $$ := $1;
    }
  ;
  
non_func_annotated_type_specifier : 
    type_specifier { $$ := $1; }
...
func_type_specifier :  -- TBD: "optional" and "queued" not handled yet
  ...
  | LAMBDA_kw operation_inputs GIVES_or_RETURN_kw func_type_operation_outputs {
	$$ := (One_Tree, Operation.Make(
	  Name => Null_Optional_Tree,
	  Operation_Kind => Operation.Func_Type_Specifier,
	  Operation_Inputs => $2.List,
...
simple_interface_element :
    non_error_simple_interface_element { $$ := $1; }
  | error_object_declaration { $$ := $1; }
  ;

non_error_simple_interface_element :
    operation_declaration { $$ := $1; }
  | non_error_object_declaration { $$ := $1; }
  | type_declaration { $$ := $1; }
  ;
...
operation_equiv :
  ...
  | func_declaration IS_kw '(' opt_class_component_list ')' {
        declare
  ...
  | func_declaration IS_kw_INDENT '(' opt_class_component_list ')' {
        declare
	    Op_Decl : Operation.Tree := 
	      Operation.Tree(Tree_Of($1.Tree));
	begin
...
class_definition_with_term :
   opt_interface_qualifier CLASS_kw module_defining_name 
      formals_and_implemented_interfaces
   L_BRACE
...
PUBLIC_kw_NEWLINE : PUBLIC_kw | PUBLIC_kw NEWLINE | PUBLIC_kw INDENT ;
...
func_declaration :
    ABSTRACT_or_QUEUED_kw basic_func_declaration {
	$$ := $2;
	declare
...
basic_func_declaration :
    VOID_kw func_designator operation_inputs opt_annotation {
	$$ := (One_Tree, Operation.Make(
	  Name => $2.Tree,
  ...
  | simple_operation_output_without_mode_or_id func_designator operation_inputs 
	...
	$$ := (One_Tree, Operation.Make(
	  ...
	  Operation_Kind => Operation.Func_Operation,
	  Operation_Inputs => $3.List,
	  Operation_Outputs => Lists.Make ((1 => $1.Tree)),
  ...
  | simple_operation_output_with_mode_without_id func_designator
      operation_inputs opt_annotation opt_AS_output_id {
        if Not_Null ($5.Tree) then
            --  Fill in name/type of output parameter
            declare
                Output_Param : Param_Decl.Tree renames Param_Decl.Tree
...
operation_input : 
    operation_input_without_mode { $$ := $1; }
  | input_mode opt_input_type_qualifier operand_type_specifier id
	    ...
	    Param_Decl.Make(
	      ...
	      Param_Type => Qualifier.Qualify
                (Qualifiers =>
                 ...
                 Operand => $3.Tree),
	      Param_Default => $6.Tree))));
    }
  | input_mode opt_input_type_qualifier
...
operation_input_without_mode :
    input_type_qualifier operand_type_specifier id
      opt_region_specifier opt_ASSIGN_expression {
	$$ := (One_List, Lists.Make ((1 =>
	    Param_Decl.Make(
	      Name => $3.Tree,
...
operation_outputs : 
--     simple_operation_output_with_id {
-- 	$$ := (One_List, Lists.Make((1 => $1.Tree)));
--     }
--   | annotation simple_operation_output_with_id {
-- 	Annotation.Add_Annotation($2.Tree, $1.List, Precedes => True);
-- 	$$ := (One_List, Lists.Make((1 => $2.Tree)));
--     }
    func_type_operation_outputs { $$ := $1; }
  ;

func_type_operation_outputs :
    simple_operation_output_without_id {
	$$ := (One_List, Lists.Make((1 => $1.Tree)));
    }
--   | annotation simple_operation_output_without_id {
-- 	Annotation.Add_Annotation($2.Tree, $1.List, Precedes => True);
-- 	$$ := (One_List, Lists.Make((1 => $2.Tree)));
--     }
...
simple_operation_output_with_id :   -- avoids trailing use of "IS"
    output_mode type_specifier id {
	$$ := (One_Tree, Param_Decl.Make(
	  Name => $3.Tree,
	  Kind => $1.Param_Kind,
	  Locking => $1.Param_Locking,
...
simple_operation_output_without_id :
    simple_operation_output_with_mode_without_id { $$ := $1; }
  | simple_operation_output_without_mode_or_id { $$ := $1; }
  ;

simple_operation_output_with_mode_without_id :
    output_mode type_specifier {
	$$ := (One_Tree, Param_Decl.Make(
	  Name => Null_Optional_Tree,
	  Kind => $1.Param_Kind,
	  Locking => $1.Param_Locking,
...
simple_operation_output_without_mode_or_id :
    type_specifier {
	$$ := (One_Tree, Param_Decl.Make(
	  Name => Null_Optional_Tree,
	  Kind => Param_Decl.Default_Param,
...
operation_output : 
    opt_output_type_qualifier operand_type_specifier id {
	$$ := (One_List, Lists.Make((1 =>
	    Param_Decl.Make(
	      Name => $3.Tree,
	      Kind => Param_Decl.Default_Param,
  ...
  | output_mode opt_output_type_qualifier operand_type_specifier {
	$$ := (One_List, Lists.Make((1 => Param_Decl.Make(
	  Name => Null_Optional_Tree,
	  Kind => $1.Param_Kind,
	  Locking => $1.Param_Locking,
...
non_error_object_declaration :
    VAR_kw type_specifier id 
      opt_region_specifier opt_ASSIGN_expression {
  ...
  | FINAL_kw id opt_region_specifier ASSIGN_or_equal expression {
	$$ := (One_Tree, Obj_Decl.Make(
  ...
  | simple_operation_output_with_mode_without_id func_designator
      opt_REFERS_TO_name {
        if Qualified_Name.Contains_String ($2.Tree) then
...
error_object_declaration :
    simple_operation_output_without_mode_or_id func_designator
	...
	$$ := (One_Tree, Obj_Decl.Make(
	  ...
	  Is_Ref => False,
	  Is_Optional => False, -- TBD
	  Obj_Type => Lists.Nth_Element ($1.List, 1),
	  Obj_Value => $3.Tree));
    }
  ;
...
object_definition :
  ...
  | output_mode id REFERS_TO name {
	$$ := (One_Tree, Obj_Decl.Make(
	  Name => PSC.Trees.Identifier.Tree(Tree_Of($2.Tree)),
	  Is_Var => $1.Param_Kind = Param_Decl.Ref_Var_Param,
	  Is_Const => $1.Param_Kind = Param_Decl.Ref_Const_Param,
	  Is_Ref => True,
	  Is_Optional => False, -- TBD
	  Obj_Type => Null_Optional_Tree,
	  Obj_Value => $4.Tree));
    }
  | FINAL_kw id MOVE movable_object {
	$$ := (One_Tree, Obj_Decl.Make(
	  Name => PSC.Trees.Identifier.Tree(Tree_Of($2.Tree)),
	  Is_Var => False,
	  Is_Const => True,
...
func_definition_with_term : 
   func_declaration L_BRACE_INDENT
  ...
  | func_declaration IS_kw queued_clause_L_BRACE
     statement_list_opt_term 
    OUTDENT_opt_NEWLINE R_BRACE_NEWLINE {
        declare
	    Op_Decl : Operation.Tree := 
...
queued_clause : 
    queued_clause_L_BRACE { $$ := $1; }
  ;

queued_clause_L_BRACE : queued_clause_condition L_BRACE_INDENT { $$ := $1; };
...
L_BRACE_INDENT : L_BRACE INDENT ; -- | NEWLINE L_BRACE INDENT ;

L_BRACE_NEWLINE : L_BRACE NEWLINE ;
...
THEN_no_indent_NL : THEN_no_indent | THEN_no_indent NEWLINE ;
...
simple_statement :
  ...
  | return_stmt { $$ := $1; }
  | CONTINUE_kw opt_id opt_WITH_values  {
	$$ := (One_Tree, Control_Stmt.Make(
  ...
  | name PLUS_PLUS {
	$$ := (One_Tree, Assign_Stmt.Make(
	  ...
	  RHS => PSC.Trees.Identifier.Make
            (PSC.Strings.String_Lookup("1"), $2.Source_Pos))); 
    }
  | name MINUS_MINUS {
	$$ := (One_Tree, Assign_Stmt.Make(
...
opt_compound_statement_kind :
  ...
  | { 
	$$ := (Construct_Kind, Control_Stmt.Any_Labeled_Stmt);
    }
  ;
...
if_statement : 
    IF_kw condition L_BRACE_INDENT
     statement_list_opt_term
...
OUTDENT_opt_else_part_END : 
    OUTDENT_R_BRACE ELSIF_or_ELSE_IF condition L_BRACE_INDENT
      statement_list_opt_term 
    OUTDENT_opt_else_part_END {
  ...
  | OUTDENT_R_BRACE ELSE_kw L_BRACE_INDENT statement_list_opt_term
      OUTDENT_R_BRACE {
	$$ := $4;
    }
  | OUTDENT_R_BRACE {
...
OUTDENT_R_BRACE : OUTDENT_opt_NEWLINE R_BRACE ;

OUTDENT_R_BRACE_NEWLINE : OUTDENT_R_BRACE NEWLINE ;

case_statement : 
    SWITCH_kw expression L_BRACE_INDENT
      case_alt_list
      opt_default_alt
    OUTDENT_opt_NEWLINE R_BRACE_NEWLINE {
	declare
...
case_alt :
    CASE_kw simple_expression_opt_named COLON_with_indent
...
simple_expression_opt_named :
    simple_expression { $$ := $1; }
  | qualified_name id {  --  Java order, type comes before ID
	$$ := (One_Tree, Param_Decl.Make(
          Name => $2.Tree,
          Kind => Param_Decl.Default_Param,
          Locking => Param_Decl.Not_Locked,
...
opt_default_alt : 
  ...
  | DEFAULT_kw id COLON_with_indent
      indented_statement_list_with_term {
	$$ := (One_Tree, Reference.Make(
	  Key => Invocation.Make(Invocation.Container_Aggregate,
	    Prefix => Null_Optional_Tree,
...
while_until_loop_statement :
    WHILE_or_UNTIL_kw condition L_BRACE_INDENT
      statement_list_opt_term
    OUTDENT_opt_NEWLINE R_BRACE_NEWLINE {
	$$ := (One_Tree, While_Stmt.Make(
          Source_Pos => $1.Source_Pos,
...
index_set_iterator :
    type_specifier id IN_kw opt_REVERSE_kw expression {
  ...
  | id IN_kw opt_REVERSE_kw expression {
	$$ := (One_Tree, Iterator.Make(
	  Kind => Iterator.Set_Iterator,
...
element_iterator :
    type_specifier id OF_no_indent expression {
	$$ := (One_Tree, Iterator.Make(
  ...
  | '[' id REFERS_TO id ']' OF_no_indent expression {
	$$ := (One_Tree, Iterator.Make(
	  ...
	  Name => PSC.Trees.Identifier.Tree(Tree_Of($4.Tree)),
	  Is_Ref => True,
	  Obj_Type => Null_Optional_Tree,
	  Obj_Value => $7.Tree,
	  Key_Name => $2.Tree));
...
initial_next_value_iterator :
    type_specifier id ASSIGN_or_equal expression 
  ...
  | type_specifier id REFERS_TO name 
...
initial_value_iterator :
    type_specifier id ASSIGN_or_equal expression opt_while_condition {
	$$ := (One_Tree, Iterator.Make(
  ...
  | type_specifier id REFERS_TO name opt_while_condition {
	$$ := (One_Tree, Iterator.Make(
	  Kind => Iterator.Initial_Value,
	  Name => PSC.Trees.Identifier.Tree(Tree_Of($2.Tree)),
	  Is_Ref => True,
  ...
  | id ASSIGN_or_equal expression opt_while_condition {
	$$ := (One_Tree, Iterator.Make(
	  Kind => Iterator.Initial_Value,
	  Name => PSC.Trees.Identifier.Tree(Tree_Of($1.Tree)),
	  Is_Ref => False,
  ...
  | id REFERS_TO name opt_while_condition {
	$$ := (One_Tree, Iterator.Make(
	  Kind => Iterator.Initial_Value,
	  Name => PSC.Trees.Identifier.Tree(Tree_Of($1.Tree)),
	  Is_Ref => True,
...
primary :
  ...
  | type_cast id_or_string_literal {
	$$ := (One_Tree, Qualified_Name.Make(
	  Prefix => $1.Tree,
	  Id => $2.Tree));
    }
...
simple_name :
    qualified_name_and_property PRIME { 
        $$ := (One_Tree, Unary.Make(Unary.Updated_Value_Op,
          Operand => $1.Tree));
    }
  | qualified_name_and_property { $$ := $1; }
  ;
...
qualified_name_and_property :
  ...
  | qualified_name_and_property Enum_Literal {
	$$ := (One_Tree, Property.Make(Operand => $1.Tree,
	  Property_Id => PSC.Trees.Identifier.Make($2.Str, $2.Source_Pos)));
    }
  ;
...
class_aggregate : 
    untyped_class_aggregate
  | type_cast '(' opt_class_component_list ')' {
...
type_cast : untyped_class_aggregate { $$ := $1; } ;
...
-- 	    Tree_Of(Lists.Nth_Element($2.List, 1))).Prefix) then
  ...
  | type_cast '[' opt_container_element_list ']' {
	-- Type of result specified
      declare
	use type Invocation.Invocation_Kind_Enum;
	Is_Double_Bracket_Special_Case : Boolean := False;
      begin
	if Lists.Length($3.List) = 1 then
	  declare
	    Solo_Operand : PSC.Trees.Tree'Class renames 
	      Tree_Ptr_Of(Lists.Nth_Element($3.List, 1)).all;
	  begin
	    if Solo_Operand in Invocation.Tree
              and then
	...
	if Is_Double_Bracket_Special_Case then
		...
		Lists.Nth_Element(Invocation.Tree(
		  Tree_Ptr_Of
                    (Lists.Nth_Element($3.List, 1)).all).Operands, 1)));
	else
...
opt_else_expr : 
    ELSIF_or_ELSE_IF condition THEN_no_indent 
      expression 
    opt_else_expr {
...
case_expr_alt : 
    CASE_kw simple_expression_opt_named COLON_no_indent expression {
	$$ := (One_Tree, Reference.Make(
	  Key => Invocation.Make(Invocation.Container_Aggregate,
	    Prefix => Null_Optional_Tree,
	    Operands => Lists.Make((1 => $2.Tree))),
  ...
  | DEFAULT_kw COLON_no_indent expression {
...
quantified_expression :
  ...
  | FOR_kw ALL_or_SOME_kw type_specifier id annotation REFERS_TO 
      condition_or_quantified_expression {
        -- This is a set iterator without the set, meaning it applies
        -- to all values of the given type, even if the type lacks
        -- a "universal" set.
  ...
  | FOR_kw ALL_or_SOME_kw id REFERS_TO 
      condition_or_quantified_expression {
        -- This is a set iterator without the set, meaning it applies
        -- to all values of the given type, even if the type lacks
        -- a "universal" set.
  ...
  | FOR_kw ALL_or_SOME_kw id annotation REFERS_TO 
      condition_or_quantified_expression {
        -- This is a set iterator without the set, meaning it applies
        -- to all values of the given type, even if the type lacks
        -- a "universal" set.
