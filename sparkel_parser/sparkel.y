...
imported_package_name_list : 
    imported_package_name {
	$$ := (One_List, Lists.Make((1 => $1.Tree)));
    }
  | imported_package_name_list ',' imported_package_name {
	$$ := $1;
...
context_clause_list : {
  ...
  | context_clause_list with_clause {
        $$ := $1;
        Lists.Append ($$.List, $2.List);
    }
  --  TBD | context_clause_list use_clause {
  --  TBD       $$ := $1;
  --  TBD       Lists.Append ($$.List, $2.List);
  --  TBD   }
  ;

with_clause : WITH_kw imported_package_name_list SEMI_or_NEWLINE {
        $$ := $2;
    }
  ;
  
--  TBD use_clause : USE_kw imported_package_name_list SEMI_or_NEWLINE {
--  TBD        $$ := (One_List, Lists.Empty_List); --  TBD
--  TBD    }
--  TBD  ;
...
package_declaration_with_term : 
 ...
 | opt_PRIVATE_kw PACKAGE_kw package_defining_name 
   ...
   OUTDENT_opt_NEWLINE opt_END_PACKAGE {
      declare
...
-- 	end if;
--     } 
--   | OPT_or_OPTIONAL_kw basic_operation_declaration opt_operation_default { 
...
polymorphic_type_name : expanded_name '+' {
...
type_instantiation : 
    package_name '<' opt_package_actual_list '>' {
	$$ := (One_Tree, Invocation.Make(
	  Kind => Invocation.Module_Instantiation,
	  Prefix => $1.Tree,
	  Operands => $3.List));
    }
  | name '[' opt_operation_actual_list ']' '<' opt_package_actual_list '>' {
...
type_qualifier :
    OPT_or_OPTIONAL_kw opt_PROTECTED_kw { 
	$$ := (Construct_Qualifier,
          Source_Pos => $1.Source_Pos,
          Is_Optional => True, 
...
pkg_spec_element_with_term : 
  ...
  | package_declaration_with_term { $$ := $1; }
  ;
...
exported_pkg_body_element_list : {
  ...
  | exported_pkg_body_element_list annotated_exported_pkg_body_element_with_term {
	$$ := $1;
	Lists.Append($$.List, $2.Tree);
    }
...
object_declaration :
    good_object_declaration { $$ := $1; }
  | missing_var_object_declaration { $$ := $1; }
  ;

good_object_declaration : 
...
missing_var_object_declaration :
    label_opt_NL
      basic_type_specifier 
...
record_definition : 
  ...
  | opt_LIMITED_or_PROTECTED_kw
      ...
      OUTDENT_opt_NEWLINE opt_END_RECORD {
	$$ := (One_Tree, Tree => PSC.Trees.Module.Make(
	  Name => Null_Optional_Tree,
	  Add_On_Label => Lists.Empty_List,
	  Is_Interface => True,
...
operation_definition_with_term : 
   ...
   OUTDENT_opt_NEWLINE opt_END_FUNC_or_PROC {
        declare
	    Op_Decl : Operation.Tree := 
	      Operation.Tree(Tree_Of($1.Tree));
	begin
...
parallel_statement :
    PARALLEL_kw INDENT
      statement_list_opt_term
    OUTDENT_opt_NEWLINE opt_END_PARALLEL {
	$$ := (One_Tree, Block_Stmt.Make(
          Source_Pos => $1.Source_Pos,
	  Block_Body => $3.Tree,
	  End_With_Values => $5.End_With_Values,
