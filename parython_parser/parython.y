...
-- operation_formal : 
--     basic_func_declaration opt_operation_default { 
...
-- 	end if;
--     } 
--   | OPT_or_OPTIONAL_kw basic_func_declaration opt_operation_default { 
...
--     } 
--   | QUEUED_kw basic_func_declaration opt_operation_default { 
-- 	if Not_Null($3.Tree) then
-- 	    $$ := (One_Tree, Operation.Add_Op_Default(
-- 	      Op_Decl => $2.Tree, Op_Default => $3.Tree));
-- 	else
...
func_type_specifier :  -- TBD: "optional" and "queued" not handled yet
  ...
  | DEF_kw operation_inputs GIVES_or_RETURN_kw operation_outputs {
	$$ := (One_Tree, Operation.Make(
	  Name => Null_Optional_Tree,
	  Operation_Kind => Operation.Func_Type_Specifier,
	  Operation_Inputs => $2.List,
...
operation_equiv :
  ...
  | op_declaration IS_kw '(' opt_class_component_list ')' {
        declare
  ...
  | op_declaration IS_kw_INDENT '(' opt_class_component_list ')' {
        declare
	    Op_Decl : Operation.Tree := 
	      Operation.Tree(Tree_Of($1.Tree));
	begin
...
op_declaration :
    opt_ABSTRACT_or_OPTIONAL_or_QUEUED_kw DEFOP_kw operator_designator 
...
basic_func_declaration :
    DEF_kw func_designator operation_inputs opt_annotation {
	$$ := (One_Tree, Operation.Make(
	  Name => $2.Tree,
  ...
  | DEF_kw func_designator operation_inputs 
...
opt_END_FUNC :
  ...
  | END_kw opt_DEF_kw func_designator SEMI_or_NEWLINE {
        $$ := (Optional_End_Token, Check_Label => True,
                Label => $3.Tree, others => Null_Optional_Tree);
    }
  ;
...
opt_END_OP :
  ...
  | END_kw opt_DEFOP_kw operator_designator SEMI_or_NEWLINE {
        $$ := (Optional_End_Token, Check_Label => True,
                Label => $3.Tree, others => Null_Optional_Tree);
    }
  ;
...
statement_list_with_term :
  ...
  | statement_list_no_term THEN_no_indent_opt_COLON_NL
  ...
  | statement_list_with_term THEN_no_indent_opt_COLON_NL
      parallel_sequence_with_term {
	-- "then" forces sequential processing; it has lower precedence
	-- than "||" so declarations preceding "then" are visible to both
	-- sides of the "||".
...
THEN_no_indent_opt_COLON_NL : THEN_no_indent | THEN_no_indent NEWLINE |
  THEN_kw COLON_no_INDENT | THEN_kw COLON_no_INDENT NEWLINE ;
...
simple_statement :
  ...
  | '(' opt_class_component_list ')' equal_as_assign expression {
	-- multiple assignment 
...
primitive_statement :
  ...
  | '(' opt_class_component_list ')' COLON_EQUAL expression {
	-- multiple assignment 
	-- NOTE: Using "opt_operation_actual_list" rather 
	--       than "operation_actual_list" to avoid ambiguity
...
OUTDENT_opt_else_part : 
    OUTDENT_ELSIF_kw condition COLON_INDENT
      statement_list_opt_term 
    OUTDENT_opt_else_part {
  ...
  | OUTDENT_ELSE_kw_COLON_INDENT statement_list_opt_term
      OUTDENT_opt_NEWLINE {
	$$ := $2;
    }
  | OUTDENT_opt_NEWLINE {
...
OUTDENT_ELSE_kw_COLON_INDENT :
    OUTDENT NEWLINE ELSE_kw COLON_INDENT ;
...
case_statement : 
    SWITCH_kw expression COLON_INDENT
      case_alt_list
      opt_default_alt
    OUTDENT_opt_NEWLINE opt_END_SWITCH {
	declare
...
opt_END_SWITCH :
  ...
  | END_kw SWITCH_kw opt_id opt_WITH_values SEMI_or_NEWLINE {
        $$ := (Optional_End_Token, Check_Label => True,
                Label => $3.Tree, End_With_Values => $4.Tree);
    }
  ;
...
--tbd   | statement_list_with_term { $$ := $1; }
  ;
...
container_element : 
    expression { $$ := $1; }
  | simple_expression COLON_no_indent expression {
	$$ := (One_Tree, Reference.Make(
	  Key => $1.Tree,
	  Referent => $3.Tree));
    }
  | dot_dot_as_interval COLON_no_indent expression {
	$$ := (One_Tree, Reference.Make(
	  Key => $1.Tree,
	  Referent => $3.Tree));
    }
  | expression FOR_kw iterator opt_annotation opt_forward_or_reverse
      {
	-- This gives an ID to an expression which can be used
	-- to parameterize the initial value expression for each element.
      declare
        Value : constant Optional_Tree := $1.Tree;
        use type PSC.Strings.U_String;
      begin
        if $5.Str /= PSC.Strings.Null_U_String then
            --  Record "forward" or "reverse" ("forward" is the default)
            Iterator.Add_Direction($3.Tree, $5.Str);
        end if;
  ...
  | simple_expression COLON_no_indent expression FOR_kw iterator
     opt_annotation opt_forward_or_reverse
      {
	-- This gives an ID to an expression which can be used
	-- to parameterize the initial value expression for each element.
      declare
        Value : constant Optional_Tree := Reference.Make(
          Key => $1.Tree, Referent => $3.Tree);
        use type PSC.Strings.U_String;
      begin

        if $7.Str /= PSC.Strings.Null_U_String then
            --  Record "forward" or "reverse" ("forward" is the default)
            Iterator.Add_Direction($5.Tree, $7.Str);
        end if;

        $$ := (One_Tree, For_Loop_Construct.Make(
          Source_Pos => $2.Source_Pos,
          Kind => For_Loop_Construct.Container_Comprehension,
          Iterators => Lists.Make((1 => $5.Tree)),
          Filter => $6.List,
          Loop_Body => Value));
      end;
    }
...
map_set_aggregate : 
    L_SET opt_container_element_list R_SET {
      declare
	use type Invocation.Invocation_Kind_Enum;
      begin
  ...
  | qualified_name DOUBLE_COLON L_SET opt_container_element_list R_SET {
	-- Type of result specified
      declare
	use type Invocation.Invocation_Kind_Enum;
      begin
...
case_expression : 
    SWITCH_kw logical_expression COLON_no_indent
      case_expr_alt_list {
	$$ := (One_Tree, Case_Construct.Make(
          Source_Pos => $1.Source_Pos,
	  Case_Selector => $2.Tree,
...
case_expr_alt : 
  ...
  | ELSE_kw COLON_no_indent expression {
